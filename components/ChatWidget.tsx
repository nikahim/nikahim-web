...
export default function ChatWidget({ userEmail = "", userName = "", embedded = false, initialOpen = false }: Props) {
  ...
  return (
    <>
      {open && chatBox}
      {/* Fotoğrafçı sayfasında launcher yok — sayfa kendi headset+destek panelini yönetir; chatBox 'nikahim:open-chat' ile açılır */}
      {!open && !hideBubble && !isPhotog && (
        ...
        >
          ...
          >
            {isCollapsed ? (
              /* Kulaklıklı mikrofon — destek anlamı (büyütüldü) */
              <svg viewBox="0 0 24 24" className="w-[28px] h-[28px]" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
