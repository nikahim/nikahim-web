...
export default function WatchPage() {
  ...
  const renderWelcomeChooser = (ready: boolean, onJoin: () => void, onPhoto: () => void) => {
    ...
    const bigVideo = <img src="/yayina-katil-line.webp" alt="" className="w-[clamp(62px,16vw,71px)] h-[clamp(52px,13vw,60px)] object-contain" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} />;
    const bigImage = <img src="/foto-ekle-line.webp" alt="" className="w-[clamp(62px,16vw,71px)] h-[clamp(52px,13vw,60px)] object-contain" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} />;
  ...
  const renderWelcomeActions = () => {
    ...
    const items = [
      { title: 'Tebrik Et', desc: 'Video, sesli veya yazılı mesaj bırakın.', icon: icTebrik, img: '/cy-tebrik2.webp', accent: rose, tint: 'rgba(201,111,120,0.10)', tab: 'tebrik' as const },
      { title: 'Altın Tak', desc: 'Çifte takmak istediğiniz altın karşılığı TL gönderin.', icon: icAltin, img: '/cy-altin.webp', accent: gold, tint: '#FAF5E8', tab: 'altin' as const },
      { title: 'Albümü Keşfet', desc: 'Fotoğrafları görün ve kendi karelerinizi paylaşın.', icon: icAlbum, img: '/cy-album2.webp', accent: rose, tint: 'rgba(201,111,120,0.10)', tab: 'album' as const },
    ];
    return (
        ...
        <div className={desktop ? 'lg:flex-1 lg:flex lg:flex-col' : ''} style={{ padding: 'clamp(15px,4.3vw,18px)', background: 'rgba(255,255,255,0.82)', border: '1px solid rgba(60,45,41,0.07)', borderRadius: 24, boxShadow: '0 12px 32px rgba(63,44,39,0.045), 0 2px 8px rgba(63,44,39,0.02)' }}>
          ...
          {count > 0 ? (
            <>
              ...
              <div onClick={() => setShowPhotoGallery(true)} className="relative w-full cursor-pointer" style={{ height: desktop ? 170 : 200, marginTop: 2 }}>
                <style>{`
                ...
                {photos[1] && (
                  <div className="alb-float absolute" style={{ left: '8%', top: 28, width: '36%', animation: 'albFloat 5.4s ease-in-out infinite' }}>
                    <div className="relative" style={{ transform: 'rotate(-5deg)' }}>
                      <img src={photos[1]} alt="" className="w-full block object-cover" style={{ height: desktop ? 132 : 150, border: '3px solid #fff', borderRadius: 15, boxShadow: '0 12px 30px rgba(55,40,32,0.08)' }} />
                ...
                {photos[2] && (
                  <div className="alb-float absolute" style={{ right: '8%', top: 28, width: '36%', animation: 'albFloat 5.4s ease-in-out infinite', animationDelay: '0.7s' }}>
                    <div className="relative" style={{ transform: 'rotate(5deg)' }}>
                      <img src={photos[2]} alt="" className="w-full block object-cover" style={{ height: desktop ? 132 : 150, border: '3px solid #fff', borderRadius: 15, boxShadow: '0 12px 30px rgba(55,40,32,0.08)' }} />
  ...
  // Sol panel yüksekliği: JS ile sabit-px SET ETME (zoom'da eski ölçüyü kilitleyip kartları bozuyordu).
  // Saf CSS lg:items-stretch hizalamayı zoom-stabil yapıyor; olası eski inline height'ı temizle.
  useEffect(() => {
    const clear = () => { if (leftPanelRef.current) leftPanelRef.current.style.height = ''; };
    clear();
  ...
  if (showReturningModal && isReturningViewer) {
    return (
      <main className="min-h-screen flex flex-col items-center px-4 pt-6 pb-10 lg:justify-center lg:pt-8" style={{ background: '#F9F2EB' }}>
        {renderDemoBlock()}
        <div className="w-full mx-auto relative overflow-hidden bg-[#FDFAF7] rounded-[22px] border border-[rgba(201,111,120,0.13)] px-4 py-6 lg:rounded-[28px] lg:px-7 lg:py-5 lg:shadow-[0_24px_70px_rgba(63,44,39,0.10)]" style={{ maxWidth: 500 }}>
          ...
          <div className="flex flex-col items-center text-center">
            <div className="rounded-full p-[3px] mb-2" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.95), rgba(201,111,120,0.35))', boxShadow: '0 12px 28px rgba(201,111,120,0.16), inset 0 1px 0 rgba(255,255,255,0.9)' }}>
              {event.couple_photo_url ? (
                <img src={event.couple_photo_url} alt="Çift" className="rounded-full object-cover block" style={{ width: 'clamp(120px,31vw,124px)', height: 'clamp(120px,31vw,124px)', border: '2px solid rgba(255,255,255,0.95)' }} />
              ) : (
                <div className="rounded-full flex items-center justify-center" style={{ width: 'clamp(120px,31vw,124px)', height: 'clamp(120px,31vw,124px)', background: '#FBEFEF', border: '2px solid rgba(255,255,255,0.95)' }}>
            ...
            <h1 className="leading-tight" style={{ color: '#1F1F1F', fontWeight: 700, fontSize: 'clamp(25px,6.5vw,28px)', letterSpacing: '-0.4px' }}>{event.bride_first_name} <span style={{ color: '#1F1F1F', fontWeight: 500, margin: '0 3px' }}>&</span> {event.groom_first_name}</h1>
            ...
            <div className="mt-2 flex items-center justify-center flex-wrap gap-x-2.5 gap-y-1 text-[15px]" style={{ color: '#8A817E' }}>
              ...
              <span className="flex items-center gap-1.5"><svg className="w-4 h-4" style={{ color: '#C96F78' }} fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{eventTime}</span>
              {event.venue && (<><span style={{ color: '#D8CFCC' }}>·</span><a href={(event as any).maps_url || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([event.venue, event.city].filter(Boolean).join(' '))}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 transition-opacity hover:opacity-75" style={{ color: '#C96F78', fontWeight: 500 }}><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" /></svg>Yol Tarifi</a></>)}
            </div>
          </div>
    ...
    return (
      <main className="min-h-screen flex flex-col items-center px-4 pt-6 pb-10 lg:justify-center lg:pt-8" style={{ background: '#F9F2EB' }}>
        ...
        <div className="w-full mx-auto relative overflow-hidden bg-[#FDFAF7] rounded-[22px] border border-[rgba(201,111,120,0.13)] px-4 py-6 lg:rounded-[28px] lg:px-7 lg:py-5 lg:shadow-[0_24px_70px_rgba(63,44,39,0.10)]" style={{ maxWidth: 500 }}>
          ...
          {/* İsim girilince beliren yönlendirme oku (kartlar aktifleşir) */}
          <div className="flex justify-center items-center" style={{ height: 24, marginTop: 12 }}>
            {(isDemoEvent || (viewerFirstName.trim().length >= 2 && viewerLastName.trim().length >= 2)) && (
              <svg className="animate-bounce" width="34" height="15" viewBox="0 0 34 15" fill="none" stroke="#C96F78" strokeWidth="2.3" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3.5l14 8 14-8" /></svg>
            )}
          </div>
          {/* İki seçim kartı */}
          <div className="mt-1">
